„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª
    StarMinerMod 0.9.9 For Minecraft1.7.10(&Forge)
    ByFARUBE(oANCIENTREDo)
„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª„ª


Thanks for downloading StarMinerMod.

This mod needs Minecraft1.7.10 with Forge in order to install&use.
Just put jar to your mod folder and you are ready for all.

This mod is still in working progress.
So be careful to use it on your mc world, cause it can mess up your wonderful world.
I strongly suggest you to use this mod on your bran new mc world.
Please do not redistribute this mod. Thanks for reading. Have fun.


[0.9.9changelog]
 -Enabled gravity attraction for some mobs.
 -Added Primodal Life Soup.
  =How to use=
   Put it to Right-up slot of GravityCore GUI.
    -Primodal Dirty/Life Soup : Star starts to Spawn mobs.
    -Soup of Intelligence : Villager on star start to mate.
 -Added Portal Item Gravity.
 -Added Mob "WanderingComet".
 -Fixed some bugs.


[0.9.8changelog]
 -Added EarthStone.
  (Able to get from creative tab only. Erase zero gravity at space dimension.)
 -Fixed Saplings Bug.
 -Added Oregen config.
 -Registered some blocks and items to ForgeOreDictionary.
 -Added fix for my new mod JointBlockMOD(Temporary name).
 -Fixed some little bugs.


[0.9.7changelog]
 -[!important for v0.9.6 users]Some recipies has changed!!
  (Gravity wall is now made from stardust. And stardust can be made from OuterStarCore)
 -Upgraded Minecraft&Forge version to 1.7.10.
 -Added G-rappleGun.
 -Added FakeRotator.
 -Added Chest(+G).
 -Added Arrow(+G).
 -Added Floating Block Gun.
 -Added stardust as intermediate material.(And changed few recipes.)
 -Added more flower&sapling(+G).
 -Fixed some bugs.


[0.9.6changelog]
 -Changed the teleporting way for GOrbit dimetion.
  Player needs to have red arrowed "GravityController" in order to teleport.
 -Lowered the height of border when teleporting to GOrbit dimetion.(before:y256+64  now:y256+32)
 -Added Item "Starbed".
 -Enabled sleep-till-morning in GOrbit dimention.(Only on single player mode)
 -Chanded rendering for fluid in GOrbit dimention.
 -Fixed "gravity freeze" symptom of other player.
 -Fixed some hardness of block.
 -Fixed lack of translation in GravityCoreBlock GUI.
 -Fixed some conflict with other MODS.

=======For MODPack Creator=======
Including my mod in modpack is contingent upon acceptance of the following terms and rules.

[Terms and rules]
 1.ARUBE is not responsible for your modpack. No support.
   If my mod caused something bad to modpack, don't try to fix it, or ask me to fix it.
   Just remove it from your modpack.
 2.Testing bugs and conflicts are totally up to modpack creator.
 3.Do not force ARUBE to fix conflict whith other mod.
   Asking for fix, fine. But it's up to ARUBE to fix or not.
   And never ever ask other mods creator to fix conflict between my mod.
 4.Right for StarminerMOD(or other mod made by ARUBE) remains to ARUBE.
 5.No direct download access from forum for modpack. Prepare your own server.
   It does not mean Starminer alone is allowed to be redistributed.
   Redistributing Starminer(or other mod made by ARUBE) alone is "STILL NOT ALLOWED!!!!!!".
 6.Deny any kind of permission for commercial purposes.(Adfly things --no comment. On your own risk.)

[Some favor from ARUBE]
 1.As far as possible, use newest version of my mod.
   Because Starminer is still in working progress, and not finished.
   But if new version make big confilict or annoying bugs, you may use old stable version.
 2.Notify Me if you included my mod in your modpack. If you can, in any way.
 3.Be aware that Starminer mod is still buggy and highly conflictable with other mods.


=======About GravityCoreBlock=======

GravityCoreBlock is placed as core of star, which is auto-generated in world.
And also, you are able to craft it, and create your own star(or cylinder type space colony).
When you are to make your own, you need to know about "star type".
Type can be chanded pressing top right button of GravityCoreBlock GUI.

Here are descriptions about each type.
 [sph(sphre type)]
   Generates sphere type gravity field.
   When terraform, It makes sphere shape.
 [cub(cubic type)]
   Generates cubic type gravity field.
   No terraform for this type.
 [xCyl,yCyl,zCyl(xyz-cylinder type)]
   Generates cylinder type repulsive field.
   When terraform, It makes cylinder shape.


So, what's Terraform?

It's a function of GravityCoreBlock helps making your own star.
First, you need to set a "star type" as I wrote above.
Second, you need to set star size(as "StarRad" in GUI, which means radius of star).
Last, place some blocks to "TerraformingMaterials" inventory.
After all is done, GravityCoreBlock starts to make a shape from y-high to y-low,
using blocks in "TerraformingMaterials" inventory.
It takes a while for small star, takes long time for bigger one.
When you need to stop function, just displace block from "TerraformingMaterials" inventory.
When you need to reset TF work, change either type or size of GravityCoreBlock.


=======ForgeOreDictionaryName=======
-Blocks
"blockGravityCore"
"blockInnerCore"
"blockOuterCore"
"blockManBazooka"
"blockNavigator"
"blockGravityWall"
"blockStarDirt"

-Items
"gemStardust"


